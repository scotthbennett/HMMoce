...
make.L.prod <- function(L1, L2 = NULL, L3 = NULL, known.locs = NULL, L.mle.res, dateVec = NULL, locs.grid = NULL, iniloc = NULL){

  stop("Error: THis is currently not functional.")
  ...
  if(is.null(L2) & is.null(L3)){
    ...
    for(ii in idx1){
      L[[ii]] = sum(L1[[ii]], L2[[ii]], L3[[ii]], na.rm = T) # when only 1 has data
    }
    
    # get these out of <1 range for prod()
    L1 <- L1 * 2
    L2 <- L2 * 2
    L3 <- L3 * 2
    
    # CHANGE 0 to 1 FOR PROD
    L1[L1 == 0] <- 1e20
    L2[L2 == 0] <- 1e20
    L3[L3 == 0] <- 1e20
    ...
    for(ii in idx3){
      ...
      L[[ii]] = prod(L1[[ii]], L2[[ii]], L3[[ii]], na.rm = T) # when all have data
    }
    
    for(ii in idx2){
      if(naL1idx[ii] & naL2idx[ii]){
        L[[ii]] <- prod(L1[[ii]], L2[[ii]], na.rm = T)
      } else if(naL1idx[ii] & naL3idx[ii]){
        L[[ii]] <- prod(L1[[ii]], L3[[ii]], na.rm = T)
      } else if(naL2idx[ii] & naL3idx[ii]){
        L[[ii]] <- prod(L2[[ii]], L3[[ii]], na.rm = T)
      }
    }
    
    for (ii in c(idx2, idx3)){
      L[[ii]][L[[ii]] == 0] <- NA
      L[[ii]][L[[ii]] == 1] <- 0
