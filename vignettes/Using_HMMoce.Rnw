...
L.rasters <- readRDS('./141259_L.rasters_20200727.rds')
res <- readRDS('./10_3_2-HMMoce_res_20200828.rds')
...
<<sp.lim_small, size='small', eval=TRUE, message=FALSE, echo=FALSE>>=
...
<<lik-light3-plot, size='small', eval=FALSE>>=
...
<<lik-light-plot, size='small', eval=TRUE>>=
  ...
  plot(L.rasters$L.light[[12]], main='Light-based location likelihood - Lon-only'); world(add=T)
...
<<lik-sst-plot, size='small', eval=TRUE>>=
  ...
  ## here's what the likelihoods look like
  plot(L.rasters$L.sst[[12]], main='SST-based likelihood'); world(add=T)
...
Here's an example of what these look like:
<<lik-pdt-plot, size='small', eval=TRUE>>=

  ## here's what the likelihoods look like
  plot(L.rasters$L.ohc[[13]], main='OHC-based likelihood'); world(add=T)
  plot(L.rasters$L.hycom.se[[13]], main='HYCOM-based likelihood'); world(add=T)
...
Here's an example of what these look like:
<<lik-bathy-plot, size='small', eval=TRUE>>=

  ## here's what the likelihoods look like
  plot(L.rasters$L.bathy[[12]], main='Bathy-based likelihood'); world(add=T)
...
<<makeL_run, size='small', cache=TRUE, eval=FALSE, echo=FALSE, message=FALSE>>=
resamp.idx <- which.max(lapply(L.rasters, FUN=function(x) raster::res(x)[1]))
L.res <- resample.grid(L.rasters, L.rasters[[resamp.idx]])
L <- make.L(L.res$L.rasters[1:3], iniloc, dateVec, known.locs = NULL)
...
Here's an example of what these look like:
<<lik-L-plot, size='small', eval=FALSE>>=

  ## here's what the likelihoods look like
  image.plot(g$lon[1,], g$lat[,1], L, main='Example overall likelihood at single time step'); world(add=T)
...
## nlminb is also supported in HMMoce but testing suggests this is rarely the best choice due to lack of convergence and other issues
...
plotRD(s, tr, ptt='141259_example', g = g, xlims = c(sp.lim$xmin, sp.lim$xmax), ylims = c(sp.lim$ymin, sp.lim$ymax), makePlot = TRUE, save.plot=FALSE)

@

<<plot-hmm, size='small', eval=FALSE, message=FALSE, echo=FALSE>>=

plotHMM(res$s, res$tr, dateVec, ptt='141259_example', save.plot = T)

plotRD(res$s, res$tr, ptt='141259_example', g = res$g, xlims = c(sp.lim$lonmin, sp.lim$lonmax), ylims = c(sp.lim$latmin, sp.lim$latmax), makePlot = TRUE, save.plot=TRUE)
