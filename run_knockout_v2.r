...
source('~/HMMoce_run/formatTracks.r')
 ...
 L.rasters$L.1 <- L.1; L.rasters$L.2 <- L.2; L.rasters$L.3 <- L.3; L.rasters$L.4 <- L.4; L.rasters$L.5 <- L.5
 resamp.idx <- which.max(lapply(L.rasters, FUN=function(x) raster::res(x)[1]))
 L.res <- resample.grid(L.rasters, L.rasters[[resamp.idx]])
 
# get idx of layers to keep
light.idx <- which(dateVec %in% knockout$blue256.light)
sst.idx <- which(dateVec %in% knockout$blue256.sst)
...
for (tt in 1:length(L.idx)){
  ...
  for (bnd in bndVec){
    for (i in parVec){
      ...
      setwd(myDir)
      write.table(tr, file=paste(ptt, '_HMM_track_knock.csv', sep=''), sep = ',', col.names = TRUE)
      df <- formatTracks(trackDir = myDir, ptt = ptt, gpeNo = gpeNo, knock=TRUE)
      res <- compareTracks(df)
      res[[4]] <- apply(res$gcd, 2, FUN=function(x) mean(x, na.rm=T))
      res[[5]] <- apply(res$gcd, 2, FUN=function(x) sd(x, na.rm=T))
      
      # WRITE OUT RESULTS
      outVec <- matrix(c(ptt=paste(ptt,'k',sep=''), minBounds = bnd, migr.spd = i, rmseLon=res$rmse.lon, rmseLat=res$rmse.lat,
