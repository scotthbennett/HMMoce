...
hmm.filter.mod <- function(g, L, K1, K2, P, bathy, maskL = T, bound.thr = 0.1, minBounds = 10){
  ...
  for(t in 2:T){
    ...
    r1 <- raster::flip(raster::raster(t(q1), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat),
                                      crs), 2)
    r2 <- raster::flip(raster::raster(t(q2), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat),
                                      crs), 2)
    if (t == 2){
    ...
    q1 <- t(raster::as.matrix(raster::flip((r1 * bathy),2)))
    q1[is.na(q1)] <- 1e-15
    q2 <- t(raster::as.matrix(raster::flip((r2 * bathy),2)))
    q2[is.na(q2)] <- 1e-15
