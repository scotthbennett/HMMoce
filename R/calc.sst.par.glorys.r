...
calc.sst.par.glorys <- function(tag.sst, filename, sst.dir, dateVec, focalDim = NULL, sens.err = 1, auto.aggr = FALSE, ncores = NULL){
  ...
  nc1 <- raster::brick(dir(sst.dir, full.names = TRUE)[1])
  ...
  ans = foreach::foreach(i = 1:T, .export = 'likint3', .packages = c('raster')) %dopar%{
    ...
    # compare sst to that day's tag-based sst
