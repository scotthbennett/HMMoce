...
```{r, eval=F, echo=F, message=F, fig.cap=''}
...
```{r, eval=F}
# RUN THE FILTER STEP
f <- hmm.filter(g, L, K1, K2, maskL=F, P.final)
...
# An example script
Here we put all of the above logic into a full working example script using the example blue shark data included with the package. Please file any issues or suggestions for improvement of `HMMoce` on the GitHub [site](https://github.com/camrinbraun/HMMoce) at https://github.com/camrinbraun/HMMoce.

```{r, eval=T, cache=T, message=T, echo=T}
library(HMMoce)

dataDir <- '~/HMMoce/inst/extdata/'
...
# READ DEPTH-TEMPERATURE PROFILE DATA
pdt <- read.wc(ptt, wd = myDir, type = 'pdt', tag=tag, pop=pop); 
pdt.udates <- pdt$udates; pdt <- pdt$data
  
# READ LIGHT DATA, OPTIONAL
#light <- read.wc(ptt, wd = myDir, type = 'light', tag=tag, pop=pop); 
#light.udates <- light$udates; light <- light$data
  
# READ GPE2 LOCATIONS FOR LIGHT-BASED LIKELIHOODS, RECOMMENDED
...
L.1 <- calc.gpe2(locs, locDates, iniloc = iniloc, locs.grid = locs.grid, dateVec = dateVec, errEll =
                     FALSE, gpeOnly = TRUE)
...
# WOA PROFILE (IN PARALLEL)
L.4 <- calc.woa.par(pdt, filename='', woa.data = woa.quarter, focalDim = 9, dateVec = dateVec, use.se = T)

# HYCOM PROFILE (IN PARALLEL)
L.5 <- calc.hycom.par(pdt, filename='sword', hycom.dir, focalDim = 9, dateVec = dateVec, use.se = T)

#----------------------------------------------------------------------------------#
# LIST, RESAMPLE, SAVE
#----------------------------------------------------------------------------------#
# CREATE A LIST OF LIKELIHOOD RASTERS
# THIS WILL ONLY WORK IF NO SPURIUS "L." VARS EXIST IN ENVIRONMENT
#L.rasters <- mget(ls(pattern = 'L\\.'))

# THIS RESAMPLES TO MOST COARSE RASTER, CHANGE IF FINER GRID IS DESIRED
resamp.idx <- which.max(lapply(L.rasters, FUN=function(x) raster::res(x)[1]))
L.res <- resample.grid(L.rasters, L.rasters[[resamp.idx]])

# GOOD IDEA TO SAVE AFTER ALL THE LIKELIHOODS
#save.image(paste(myDir, ptt, '_likelihoods.RData', sep=''))
  
#----------------------------------------------------------------------------------#
# COMBINE LIKELIHOOD MATRICES
#----------------------------------------------------------------------------------#
# PASS THE DESIRED RASTERS TO MAKE.L WITH THE L1 ARGUMENT
# HERE WE USE LIGHT, SST, OHC (1, 2, 3)
L <- make.L(L1 = L.res[[1]][c(1:3)], L.mle.res = L.res$L.mle.res, dateVec = dateVec, locs.grid = locs.grid, iniloc = iniloc, bathy = bathy, pdt = pdt)
