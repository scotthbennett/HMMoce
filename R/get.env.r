...
get.env <- function(uniqueDates = NULL, filename = NULL, type = NULL, spatLim = NULL, resol = NULL, save.dir = getwd(), sst.type=NULL, depLevels=NULL, ...){
  ...
  if(is.null(type)){
    ...
    for(i in 1:length(uniqueDates)){
      ...
      if (spatLim$lonmax > 180 & time < as.Date('2013-01-01')){ ## 0 to 360
        cat('Detected input coordinates > 180, downloading multiple files.','\n')
        ex180 <- raster::extent(-180,180,-90,90)
        ex360 <- raster::extent(180,360,-90,90)
      ...
      } else if (spatLim$lonmin < 0 & time >= as.Date('2013-01-01')){ ## trying to request using 180 coords from 360 lons in HYCOM
      ...
      } else if (spatLim$lonmax > 180 & time >= as.Date('2013-01-01')){ ## 0 to 360
        cat('HYCOM outputs are 0-360 starting in 2013. Adjusting by downloading multiple files.','\n')
        ...
        raster::writeRaster(r1, paste(save.dir, '/', filename, '_', time, '.nc', sep = ''), format='CDF', overwrite=TRUE, varname = ncnames[grep('temp', ncnames, ignore.case=TRUE)])
        if (file.exists(paste(save.dir, '/', filename, '_', time, '.nc', sep = ''))) cat(paste0('File output to ', paste(save.dir, '/', filename, '_', time, '.nc', sep = '')),'\n')
