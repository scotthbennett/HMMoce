...
\title{\textbf{HMMoce: An R package for improved geolocation of archival‐tagged fishes using a hidden Markov method}}
...
The package is presented in \cite{braun2018a}, where its use is illustrated on example blue and mako shark PSAT data as well as compared to other commonly employed methods for geolocation of these and similar data. A subset of the data used in that paper is included as example data in the package.
...
Here we illustrate the use of the package with an example dataset from a blue shark tagged with a Wildlife Computers miniPAT in the NW Atlantic. Depending on which observation likelihoods (\ref{sec:lik}) you choose to generate for your species of interest, tag-based data streams like SST may be more or less useful. Our example dataset is from a blue shark which tends to occupy the epipelagic where the tag is able to consistently record high quality light and SST data that can be used for geolocation. Furthermore, this species tends to also make daily excursions, sometimes to $>$1000 m, which yields water column temperature profiles that can also be leveraged to improve geolocation estimates. Thus,...

\subsection{Loading the tag data} \label{tag-data}

First, setup the start/end dates and locations. These are then used to build a vector of POSIXct timestamps that represent the desired time steps of the likelihoods and, ultimately, most probable track.
<<iniloc,size='small'>>=
...
Next, we load some tag data. Raw example data is included with the package and can be read by pointing to the "extdata" directory associated with the install of the \texttt{HMMoce} package. Helper functions are provided for directly reading data from Wildlife Computers tags, however the package works with any tag data that can be coerced to the minimum required input data for the various observation likelihoods. The output contains three columns: "Date", "Depth", and "Temperature". Date must be of class POSIXct. Depth represents the depth at which the corresponding sea surface temperature ("Temperature") was taken. Currently the column "Depth" is not used but is retained here for user convenience in case, for example, it makes sense to filter the SST data due to SSTs being recorded deeper than what makes sense given the oceanography the tagged animal is in. The \texttt{read.wc()} helper functions all also output a unique dates vector, here called sst.udates. This is only necessary for downloading some representation of the SST field (\ref{sec:env}) which we compare the tag data to for building the observation likelihoods.
...
## sst.udates are the unique DAYS contained in the tag SST data. These are used to only download environmental SST data for which the tag reported SST (to save space/time).
...
- temporal resolution of your summarized dataset can only be as high as the resolution of your `dateVec` object that is the "temporal backbone" of this entire modeling process. Thus, it makes sense that the timescales and resolution of these match, if possible.
...
<<light_raw,size='small'>>=

lightFile <- system.file("extdata", "141259-LightLoc.csv", package = "HMMoce")
light <- read.wc(lightFile, type = 'light', tag=tag, pop=pop, verbose=T)$data
...
\subsubsubsection{Light-based position estimates}
...
ll <- read.table(llFile, sep = ',', header = T, blank.lines.skip = F)
ll$Date <- as.POSIXct(ll$Date, format = findDateFormat(ll$Date))
@


\subsubsection{Spatial limits}
The last data preparation step is to establish the spatial bounds of this model run. How you do this can vary widely based on species, their movement ecology, and the quality of the tag data used. In general, a spatial buffer of at least a few degrees (and sometimes many!) around the start/end locations is a good place to start. These limits are used for downloading environmental data and, ultimately, serve as the bounds for the model. Thus, err on the side of making the bounds too large to be sure the extent of movements are adequately captured. The tradeoff as model bounds expand is, of course, computational demands increase as larger grids are computed. In the end, the choice of spatial bounds comes down to expert opinion so choose carefully. Spatial bounds are then used to setup the basic spatial grid the remaining calculations will be based on.
<<splim,size='small'>>=
# SET SPATIAL LIMITS
...
There are a number of SST products out there that meet our needs. While modeled SST products work fine, we typically use remote-sensing products. There are 3 products currently built into the \texttt{get.env} functionality within \texttt{HMMoce}. Here we'll use the Optimum Interpolation SST (OISST) from NOAA. Note the tradeoffs associated with different SST products such as gaps, gap-filling techniques/interpolations, and product resolution.

<<env-sst,size='small'>>=
sst.dir <- paste0(dir, 'EnvData/sst/', sep='')
...
As with SST above, there are a number of ways to represent temperature of the 3D ocean. While observations are nice, they are typically too sparse to generate a reliable grid on which we can calculated likelihoods. Thus, we really only have 2 good options for most scenarios: climatological mean temperature and oceanographic models. If using the latter, data-assimilating models constantly "nudge" the model outputs to match reality and are typically a better choice for our purposes. \texttt{HMMoce} includes functionality for accessing the World Ocean Atlas climatology at 1/4$^\circ$ and 1$^\circ$ and for downloading HYCOM model data. For most applications, models such as HYCOM are more likely to generate realistic likelihood results when comparing \textit{in situ} tag data to a dynamic ocean.

<<env-hycom,size='small'>>=
...
World Ocean Atlas is all but deprecated but is still included for the rare occasion it may prove more useful than other ways to represent the 3D ocean.
<<env-woa,size='small'>>=
...
<<env-bathy,size='small'>>=

bathy.dir <- paste0(dir, 'EnvData/bathy/', sep='')
